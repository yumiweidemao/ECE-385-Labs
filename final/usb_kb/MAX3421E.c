...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	/* Write command byte to slave (MAX3421E),
	 * Command byte: {REG4, REG3, REG2, REG1, REG0, 0, !R/W, ACKSTAT(unused)}
	 * Note that input reg is already left shifted by 3 bits.                  */
	BYTE command_byte = (reg | 0x02);
	BYTE arr[2] = {command_byte, val};
...
BYTE MAXreg_rd(BYTE reg) {
	...
	int ret_code;
	ret_code = alt_avalon_spi_command(SPI0_BASE, 0,
									  1, &reg,				// write command byte = reg
									  1, &rx_data,			// read length = 1 byte
									  0);
	if (ret_code < 0) {
